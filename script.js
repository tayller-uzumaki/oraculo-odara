    ...
    formOdu.addEventListener('submit', function (e) {
      ...
      setTimeout(() => {
        ...
        const textoCaminhoExpandido = `
          <p style="margin-bottom: 10px; color: #CBD5E0; line-height: 1.6;">
            ${oduDiretorAtual.caminho} Sob a regência máster de <strong style="color: #D4AF37;">${oduDiretorAtual.orixa}</strong>, este Odù atua através do elemento <strong>${oduDiretorAtual.elemento}</strong>, moldando os ciclos de crescimento, superação e colheita ao longo da sua jornada terrena.
          </p>
          <p style="margin-bottom: 12px; color: #CBD5E0; line-height: 1.6;">
          ...
          <div style="margin-top: 12px; padding: 14px; background: rgba(255,255,255,0.03); border-radius: 8px; border: 1px solid rgba(255,255,255,0.1);">
            <p style="color: #68D391; margin-bottom: 6px; font-size: 13px;"><strong>✨ Pontos Fortes & Potenciais:</strong> ${oduDiretorAtual.fatoresFavoraveis.join(' • ')}.</p>
            <p style="color: #FC8181; margin-bottom: 0; font-size: 13px;"><strong>⚠️ Pontos de Alerta & Cuidados:</strong> ${oduDiretorAtual.pontosAtencao.join(' • ')}.</p>
          </div>
        `;
...
function gerarPix() {
  ...
  container.innerHTML = `
    <div style="background: rgba(255,255,255,0.05); padding: 16px; border-radius: 8px; border: 1px solid rgba(212,175,55,0.2); text-align: center; margin-top: 12px;">
      <p style="color: #E2E8F0; font-size: 14px; margin-bottom: 8px; font-weight: 600;">Copia e Cola PIX (R$ ${pacoteAtivo.valor.toFixed(2)}):</p>
      <input type="text" value="${qrSimulado}" readonly style="width: 100%; padding: 10px; font-size: 12px; background: rgba(0,0,0,0.3); color: #D4AF37; border: 1px solid rgba(212,175,55,0.3); border-radius: 6px; margin-bottom: 12px; text-align: center;" />
...
// ==========================================
// SVG DE BÚZIOS (COM TAMANHO CORRIGIDO)
// ==========================================
function criarBuzioSVG(eAberto) {
  if (eAberto) {
    return `
      <svg viewBox="0 0 50 70" style="width: 100%; height: 100%; display: block;">
        <defs>
...
// MOTOR DE INTERPRETAÇÃO NO TEMA ESCURO HARMONIZADO
function gerarInterpretacaoDinamicamente(perguntaText, oduObj, area) {
  ...
  if (ehDemanda) {
    notaAvisoAdicional = `<div style="margin-top: 12px; padding: 12px; background: rgba(254,178,178,0.1); border: 1px solid #FEB2B2; border-radius: 6px; font-size: 12px; color: #FEB2B2; line-height: 1.5;">
    ...
    notaAvisoAdicional = `<div style="margin-top: 12px; padding: 12px; background: rgba(226,232,240,0.1); border: 1px solid #E2E8F0; border-radius: 6px; font-size: 12px; color: #CBD5E0; line-height: 1.5;">
  ...
  return `
    <div style="background: rgba(255,255,255,0.03); padding: 18px; border-radius: 8px; border: 1px solid rgba(255,255,255,0.1); margin-bottom: 16px;">
      <span style="font-size: 11px; color: #D4AF37; font-weight: bold; text-transform: uppercase; letter-spacing: 1px;">3. Interpretação Aprofundada</span>
      <p style="font-size: 13px; color: #A0AEC0; margin: 4px 0 12px 0; font-style: italic;">"Pergunta feita: ${perguntaText}"</p>
      
      <div class="box-destaque-dark">
        <p style="font-size: 14px; color: #E2E8F0; line-height: 1.6; margin: 0;">${direcionamento}</p>
      </div>

      <p style="font-size: 13px; color: #CBD5E0; line-height: 1.5; margin-bottom: 8px;">
...
// ==========================================
// ETAPA 3: JOGADA DE BÚZIOS COM DESIGN ESCURO
// ==========================================
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (formConsulta) {
    formConsulta.addEventListener('submit', function (e) {
      ...
      // ============================================================
      // PROTOCOLO DE SEGURANÇA: INTERCEPTAÇÃO DE IDEAÇÃO
      // ============================================================
        ...
        setTimeout(() => {
          ...
          if (resLeitura) {
            resLeitura.innerHTML = `
              <div class="box-alerta-seguranca" style="max-width: 700px; margin: 0 auto;">
                <div style="text-align: center; margin-bottom: 20px;">
                  <span style="font-size: 36px;">🕊️</span>
                  <h3 style="color: #FC8181; font-size: 1.3rem; margin-top: 6px; font-weight: 700;">Mensagem de Acolhimento e Apoio</h3>
                </div>

                <div style="margin-bottom: 16px; padding: 14px; background: rgba(0,0,0,0.2); border-left: 4px solid #E53E3E; border-radius: 4px;">
                ...
                <div style="margin-bottom: 16px; padding: 14px; background: rgba(0,0,0,0.2); border-left: 4px solid #3182CE; border-radius: 4px;">
                  <p style="color: #E2E8F0; font-size: 0.95rem; line-height: 1.6; margin: 0;">
                ...
                <div style="margin-bottom: 16px; padding: 16px; background: rgba(0,0,0,0.3); border-radius: 8px; border: 1px solid rgba(255,255,255,0.1);">
                  <h4 style="color: #63B3ED; font-size: 0.95rem; margin-bottom: 6px; font-weight: 700;">📞 Canal de Apoio Emocional (CVV)</h4>
                  <p style="color: #E2E8F0; font-size: 0.9rem; line-height: 1.5; margin: 0;">
                    No Brasil, você pode entrar em contato gratuitamente com o <strong>CVV (Centro de Valorização da Vida)</strong> pelo telefone <strong>188</strong>, disponível 24 horas por dia, ou acessar <a href="https://www.cvv.org.br" target="_blank" rel="noopener noreferrer" style="color: #63B3ED; text-decoration: underline; font-weight: 600;">www.cvv.org.br</a>.
                  </p>
                </div>

                <div style="margin-bottom: 16px; padding: 14px; background: rgba(0,0,0,0.2); border-left: 4px solid #D69E2E; border-radius: 4px;">
                  <p style="color: #E2E8F0; font-size: 0.9rem; line-height: 1.5; margin: 0;">
                    🌿 <strong>Orientação Espiritual:</strong> Se fizer sentido para você, também pode buscar orientação com uma Ialorixá ou Babalorixá de confiança, que poderá oferecer acolhimento espiritual responsável.
                  </p>
                </div>

                <div style="margin-bottom: 20px; padding: 14px; background: rgba(0,0,0,0.2); border-left: 4px solid #805AD5; border-radius: 4px;">
                  <p style="color: #E2E8F0; font-size: 0.9rem; line-height: 1.5; margin: 0;">
      ...
      setTimeout(() => {
        ...
        if (peneira) {
          ...
          for (let i = 0; i < 16; i++) {
            ...
            buzioDiv.className = 'buzio-item'; // Classe que trava a dimensão
        ...
        if (resLeitura) {
          resLeitura.innerHTML = `
            <div class="card-resultado-dark">
              <div style="border-bottom: 2px solid #D4AF37; padding-bottom: 10px; margin-bottom: 16px;">
                <span style="font-size: 11px; color: #D4AF37; font-weight: bold; text-transform: uppercase; letter-spacing: 1px;">Veredito dos Búzios</span>
                <h3 style="color: #FFF; margin: 4px 0;">${oduJogo1.tituloTendencia}</h3>
                <p style="color: #CBD5E0; font-size: 0.9rem; margin: 0;">Odù Revelado: <strong>#${oduJogo1.numero} — ${oduJogo1.nome}</strong> (Orixá: ${oduJogo1.orixa})</p>
              </div>

              ${gerarInterpretacaoDinamicamente(pergunta, oduJogo1, area)}

              <div style="margin-bottom: 16px; background: rgba(255,255,255,0.03); padding: 14px; border-radius: 6px; border: 1px solid rgba(255,255,255,0.1);">
                <h4 style="color: #D4AF37; font-size: 0.95rem; margin-bottom: 6px;">🛡️ Influências Espirituais & Orixás:</h4>
                <p style="color: #CBD5E0; font-size: 0.9rem; line-height: 1.5; margin: 0;">${oduJogo1.influenciaEspiritual}</p>
              </div>

              <div style="margin-bottom: 20px; background: rgba(255,255,255,0.03); padding: 14px; border-radius: 6px; border: 1px solid rgba(255,255,255,0.1);">
                <h4 style="color: #D4AF37; font-size: 0.95rem; margin-bottom: 6px;">💡 Sabedoria Ancestral & Conselho:</h4>
                <p style="color: #CBD5E0; font-size: 0.9rem; line-height: 1.5; margin: 0;"><em>"${oduJogo1.sabedoriaAncestral}"</em></p>
              </div>

              <div style="background: rgba(0,0,0,0.2); padding: 16px; border-radius: 8px; text-align: center;">
                <p id="status-email-envio" style="font-size: 0.85rem; color: #E2E8F0; margin-bottom: 12px; display: none;"></p>
