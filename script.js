...
document.getElementById('form-odu')?.addEventListener('submit', function (e) {
  ...
  const nomeInput = document.getElementById('nome')?.value || 'Consultante';
  ...
  setTimeout(() => {
    ...
    const elemNum = document.getElementById('odu-numero');
    const elemNome = document.getElementById('odu-nome');
    const elemOrixa = document.getElementById('odu-orixa');
    const elemElem = document.getElementById('odu-elemento');

    if (elemNum) elemNum.innerText = oduDiretorAtual.numero;
    if (elemNome) elemNome.innerText = oduDiretorAtual.nome;
    if (elemOrixa) elemOrixa.innerText = oduDiretorAtual.orixa;
    if (elemElem) elemElem.innerText = oduDiretorAtual.elemento;

    const textoCaminhoExpandido = `
      <p style="margin-bottom: 10px; line-height: 1.6;">
        Olá, <strong>${nomeInput}</strong>! ${oduDiretorAtual.caminho} Sob a regência ancestral do Orixá <strong>${oduDiretorAtual.orixa}</strong> e regido pela vibração do elemento <strong>${oduDiretorAtual.elemento}</strong>, este Odù atua como o mapa da sua jornada terrena.
      </p>
    ...
    const elemCaminho = document.getElementById('odu-caminho');
    if (elemCaminho) elemCaminho.innerHTML = textoCaminhoExpandido;
...
function selecionarPacote(qtd, valor) {
  ...
  const p5 = document.getElementById('pacote-5');
  const p10 = document.getElementById('pacote-10');
  if (p5) p5.classList.toggle('active', qtd === 5);
  if (p10) p10.classList.toggle('active', qtd === 10);
}
...
function confirmarPagamento() {
  ...
  const secJogada = document.getElementById('secao-jogada');
  if (secJogada) {
    secJogada.style.display = 'block';
    secJogada.scrollIntoView({ behavior: 'smooth' });
  }
}
