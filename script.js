...
let perguntasRestantes = 4;

// Função Visual para Desenhar o Búzio com Degradê e Dourado Realista
function criarBuzioSVG(eAberto) {
  if (eAberto) {
    // Búzio Aberto (Fundo claro com bordas douradas e fenda interior)
    return `
      <svg viewBox="0 0 50 70" width="100%" height="100%">
        ...
        <path d="M25 2 C 42 2, 48 18, 47 35 C 46 52, 38 68, 25 68 C 12 68, 4 52, 3 35 C 2 18, 8 2, 25 2 Z" fill="url(#gradAberto)" stroke="#fef08a" stroke-width="1.8"/>
        <path d="M25 10 C 22 18, 22 52, 25 60 C 28 52, 28 18, 25 10 Z" fill="#2e1065"/>
    ...
    // Búzio Fechado (Costas Douradas brilhantes)
    return `
      <svg viewBox="0 0 50 70" width="100%" height="100%">
        <defs>
          <linearGradient id="gradFechado" x1="0%" y1="0%" x2="100%" y2="100%">
        ...
        <path d="M25 3 C 41 3, 47 19, 46 35 C 45 51, 37 67, 25 67 C 13 67, 5 51, 4 35 C 3 19, 9 3, 25 3 Z" fill="url(#gradFechado)" stroke="#fef08a" stroke-width="1.5"/>
        <path d="M25 6 C 29 20, 29 50, 25 64" stroke="#ffffff" stroke-width="1.2" stroke-dasharray="2 3" opacity="0.6" fill="none"/>
      </svg>
    `;
  }
}

// Evento Principal de Consulta
document.getElementById('form-consulta')?.addEventListener('submit', function (e) {
  ...
  const pergunta = perguntaInput ? perguntaInput.value : "Sua pergunta";
  ...
  for (let i = 0; i < 16; i++) {
    ...
    const top = Math.floor(Math.random() * 58 + 20);
    const left = Math.floor(Math.random() * 58 + 20);
    const rot = Math.floor(Math.random() * 360);

    buzioEl.style.top = `${top}%`;
    buzioEl.style.left = `${left}%`;
    buzioEl.style.transform = `rotate(${rot}deg)`;
    buzioEl.style.animationDelay = `${i * 0.05}s`;

    peneira?.appendChild(buzioEl);
  }

  // Atualização dos Textos de Animação
  ...
  // Exibição e Geração Final do Resultado sem Erros
  setTimeout(() => {
    perguntasRestantes--;
    ...
    if (resContainer) {
      resContainer.innerHTML = `
        <div style="display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid #332147; padding-bottom: 10px; margin-bottom: 14px;">
          <span style="font-size: 12px; color: #fbbf24; font-weight: bold; background: #200f38; padding: 4px 10px; border-radius: 20px; border: 1px solid #fbbf24;">
        ...
        <div style="margin: 16px 0; background: #0b0612; padding: 14px; border-radius: 8px; border: 1px solid #332147;">
          ...
          <p style="font-size: 13px; color: #d4d4d8; line-height: 1.6;">
            A queda de ${numAbertos} búzios traz a regência direta do Orixá <strong>${oduJogo.orixa}</strong>, ativando energias do elemento <strong>${oduJogo.elemento}</strong>.
          </p>
        </div>
        ...
        <div style="margin: 16px 0; background: #0b0612; padding: 14px; border-radius: 8px; border: 1px solid #f59e0b;">
          ...
          <p style="font-size: 13.5px; color: #fff; line-height: 1.6;">
            Em relação à sua dúvida, o oráculo revela que o cenário atual pede clareza. A energia de ${oduJogo.nome} indica que ${oduJogo.favorabilidade > 60 ? 'existem caminhos abertos e suporte favorável para o desfecho que você busca.' : 'existem obstáculos e pendências que precisam ser tratados com cautela antes de tomar atitudes.'}
          </p>
        </div>
