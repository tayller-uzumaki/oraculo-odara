...
function classificarBloqueio(texto) {
  ...
  if (
    ...
    return {
      ...
      mensagem:
        'O que você escreveu precisa de acolhimento humano imediato, não de uma leitura dos búzios. ' +
        'Por isso, esta consulta não será realizada e nenhum crédito será consumido. ' +
        'Procure agora uma pessoa adulta de confiança e apoio profissional ou um serviço de emergência da sua região.'
    };
  }
  ...
  // -------------------------------------------------------
  // D. DIAGNÓSTICO, TRATAMENTO E PROMESSA DE CURA
  // -------------------------------------------------------

  const padroesDiagnosticoDireto = [

    /\bqual (e )?(a )?minha doenca\b/,

    /\bqual doenca eu tenho\b/,

    /\bque doenca eu tenho\b/,

    /\bqual (e )?meu diagnostico\b/,

    /\bque problema de saude eu tenho\b/,

    /\bqual problema de saude eu tenho\b/,
    ...
    /\btenho alguma doenca\b/,

    /\bvou me curar\b/,

    /\bvou ficar curad[oa]\b/,

    /\bessa doenca vai sumir\b/,

    /\bposso parar meu remedio\b/,

    /\bdevo parar meu remedio\b/,

    /\bqual remedio devo tomar\b/,

    /\bqual medicamento devo tomar\b/

  ];


  const diagnosticoDireto =
    padroesDiagnosticoDireto.some(
      padrao =>
        padrao.test(t)
    );


  const mencionaOraculo =
    /\b(buzios|oraculo|odu|jogo)\b/
      .test(t);


  const mencionaCondicaoMedica =
    /\b(doenca|diagnostico|cancer|tumor|enfermidade|problema de saude|condicao medica)\b/
      .test(t);


  const pedeIdentificacaoMedica =
    /\b(qual|que|dizer|saber|descobrir|identificar|diagnosticar|tenho|estou com)\b/
      .test(t);


  const diagnosticoViaOraculo =
    mencionaOraculo &&
    mencionaCondicaoMedica &&
    pedeIdentificacaoMedica;


  const estruturaDiagnostica =
    /\b(qual|que)\b/.test(t) &&
    /\b(doenca|diagnostico|problema de saude|condicao medica)\b/.test(t) &&
    /\b(eu tenho|tenho|meu|minha)\b/.test(t);


  if (
    diagnosticoDireto ||
    diagnosticoViaOraculo ||
    ...
    return {
      ...
      mensagem:
        'O Oráculo pode acolher reflexões sobre seu momento e bem-estar, mas não realiza diagnóstico médico, não determina tratamento e não promete cura. ' +
        'Questões de saúde precisam ser avaliadas por profissionais qualificados. ' +
        'Se quiser consultar os búzios nesse contexto, reformule a pergunta para aspectos simbólicos de cuidado, equilíbrio ou caminhos para atravessar esse momento. ' +
...
function detectarSensibilidades(texto) {
  ...
  // -------------------------------------------------------
  // DEMANDA / FEITIÇO / INVEJA / CAMINHOS TRAVADOS
  // -------------------------------------------------------

  if (
    /\bdemanda\b|\bfeitico\b|\bmacumba\b|\binveja\b|\btrabalho contra mim\b|\benergia negativa\b|\binfluencia espiritual negativa\b|\bcaminhos? travad[oa]s?\b|\bvida travada\b|\bprote[cç]ao espiritual\b/
      .test(t)
  ) {
  ...
  if (
    /\bprocesso\b|\bjustica\b|\badvogad[oa]\b|\bindenizacao\b|\bacordo judicial\b|\bcausa trabalhista\b/
      .test(t)
  ) {
  ...
  if (
    /\bdivida\b|\bendividad[oa]\b|\bsem dinheiro\b|\bfalencia\b|\bperder dinheiro\b/
      .test(t)
  ) {
...
function classificarIntencao(pergunta) {
  ...
  //
  // TEM PRIORIDADE SOBRE ESPIRITUALIDADE GENÉRICA.
  // -------------------------------------------------------
  ...
  const perguntaOrixas = [
    ...
    /\bforca principal\b.*\borixa/,

    /\borixa.*\bforca principal\b/,

    /\bforca complementar\b.*\borixa/,

    /\borixa.*\bforca complementar\b/,

    /\bforca ancestral\b.*\borixa/,

    /\borixa.*\bforca ancestral\b/
  ...
  const descreveTrincaDeForcas =
    mencionaOrixa &&
    /\b(principal|complementar|ancestral|regencia|regencias|momento|frente|cabeca|eleda|junto|adjunto)\b/
      .test(t);
  ...
  //
  // IMPORTANTE:
  // TODOS OS TERMOS TÊM LIMITES DE PALAVRA.
  //
  // A versão anterior continha "ex" sem limites,
  // então palavras como "existe" podiam ser
  // classificadas incorretamente como relacionamento.
  // -------------------------------------------------------

  if (
    /\b(amor|relacionamento|namoro|namorada|namorado|casamento|marido|esposa|parceiro|parceira|traicao|voltar|separacao|terminar)\b|\bex\b/
      .test(t)
  ) {
  ...
  if (
    /\b(processo|justica|advogado|advogada|indenizacao|audiencia)\b|\bcausa trabalhista\b/
      .test(t)
  ) {
  ...
  // -------------------------------------------------------
  // G. ESPIRITUALIDADE E PROTEÇÃO
  // -------------------------------------------------------

  if (
    /\b(espiritual|espiritualidade|protecao|inveja|demanda|energia|axe|feitico|macumba|ancestral|terreiro)\b|\benergia negativa\b|\binfluencia espiritual negativa\b|\bcaminhos? travad[oa]s?\b|\bvida travada\b/
      .test(t)
  ) {
...
==================================================
4. RESPOSTA DIRETA ANTES DA EXPLICAÇÃO
==================================================

Depois de reconhecer brevemente a pergunta, entregue cedo a conclusão principal da leitura.

Evite obrigar a pessoa a ler vários parágrafos para descobrir se a tendência é favorável, cautelosa ou desfavorável.

Prefira construções como:

"A queda é favorável para esta questão, mas pede atenção a..."

"A queda não confirma esse temor; o principal alerta está em..."

"Neste momento, a tendência é desfavorável porque..."

Depois explique o fundamento no Odù e os pontos de responsabilidade pessoal.


==================================================
5. POSITIVIDADE TÓXICA É PROIBIDA
==================================================
...
IMPORTANTE: a associação entre Odù e Orixá recebida nos dados é uma correspondência usada nesta leitura digital. Não trate essa correspondência, por si só, como confirmação ritual de Orixá pessoal.

Nas três quedas, prefira dizer "força apresentada nesta queda" ou "Orixá associado à queda". A leitura pode integrar as três forças do momento, mas não deve convertê-las automaticamente em definição permanente de cabeça, juntó ou ancestralidade ritual.
